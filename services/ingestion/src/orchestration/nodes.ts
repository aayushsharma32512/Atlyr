...
export async function crawlNode(state: PipelineState): Promise<Partial<PipelineState>> {
  ...
  const result = await firecrawlLimiter(() => fetchWithFirecrawl(crawlPayload.originalUrl, config.FIRECRAWL_API_KEY, {
    scrape: {
      prompt: plan.prompt,
...
export async function garmentSummaryNode(state: PipelineState): Promise<Partial<PipelineState>> {
  ...
  const summaryImages = rawImages
    .filter((img) => Boolean(img.summaryEligible) && (img.productView === 'front' || img.productView === 'back'))
    .map((img) => {
...
export async function ghostNode(state: PipelineState): Promise<Partial<PipelineState>> {
  ...
  const rawImages = collectRawImages(state);
  const targets = rawImages.filter((img) => Boolean(img.ghostEligible) && (img.productView === 'front' || img.productView === 'back'));
  if (!targets.length) {
