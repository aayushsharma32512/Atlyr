...
async function searchAlternatives({
  ...
  const hasActiveSearch = (query && query.trim().length > 0) || (imageUrl && imageUrl.trim().length > 0)
...
async function getOutfitsByProduct({
  ...
  const response = (await (supabase as any)
    ...
    .limit(limit)
    .order("popularity", { ascending: false })) as { data: DbOutfitRow[] | null; error: PostgrestError | null }
