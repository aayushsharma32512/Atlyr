...
async function searchOutfits({
  ...
  const { results, nextCursor } = await invokeSearchFunction<Record<string, unknown>>({
    query,
    searchType: "outfits",
    gender,
    cursor,
  ...
  const ordered: OutfitSearchResult[] = ids
    .map((id, index) => {
      ...
      const similarity = typeof results[index]?.["similarity"] === "number" ? (results[index]["similarity"] as number) : undefined
      return {
        outfit: entry.outfit,
  ...
  return { results: ordered, nextCursor }
}
...
async function getProductFilterOptions(typeFilters?: Database["public"]["Enums"]["item_type"][]): Promise<ProductFilterOptions> {
  ...
  let filteredQuery = supabase
    .from("products")
    .select("gender, brand, category_id, fit, feel, vibes")
