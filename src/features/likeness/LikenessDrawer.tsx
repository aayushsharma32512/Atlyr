...
export function LikenessDrawer({
  ...
  const listQuery = useLikenessListQuery()
  ...
  const handleGenerateLikeness = useCallback(async () => {
    ...
    try {
      ...
      // Add job to tracker IMMEDIATELY so user sees progress hub right away
      addJob({
        id: tempId,
        type: "likeness",
        status: "processing",
      ...
      // Move to step 2 after upload starts
      updateStep(2)
    } catch (error) {
      toast({
        title: "Upload failed",
  ...
  }, [form, toast, uploadMutation, updateStep, addJob, updateJob, outfitItems, outfitSnapshot])
  ...
  const handleUseAvatar = useCallback(
    async (poseId: string) => {
      ...
      try {
        // Add temporary job immediately for instant feedback
