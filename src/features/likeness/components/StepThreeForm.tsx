...
import { Loader2, Maximize2, Share, Rotate3d } from "lucide-react"
...
export function StepThreeForm({
  ...
  return (
    <Form {...form} >
      <div className={`flex flex-1 flex-col min-h-0 h-full w-auto mx-auto ${type === 'drawer' ? 'border-none shadow-none' : ''}`}>
        <CardHeader className={`flex gap-3 items-center justify-center h-auto p-6 shrink-0 ${type === 'drawer' ? 'border-none shadow-none' : ''}`}>
          <div className="flex flex-1 flex-col gap-1.5 items-center justify-center text-center">
            <p className="text-base font-medium text-card-foreground leading-none">Select Base User Avatar</p>
              ...
              <div key={pose.id} className="flex flex-col gap-2 items-center relative w-full max-w-[200px]">
                <button
                  ...
                  className={`bg-muted flex flex-col items-end justify-end px-2.5 py-3 relative rounded-[10px] w-full aspect-square transition-all hover:opacity-90 ${
                    selectedAvatar === pose.id ? "ring-2 ring-primary ring-offset-2" : ""
                  }`}
                >
        ...
        {savedMode ? (
          <CardFooter className={`flex gap-2 items-center justify-center pb-6 pt-2.5 px-3 shrink-0 ${type === 'drawer' ? 'border-none shadow-none' : ''}`}>
            <Button
              type="button"
          ...
          <CardFooter className={`flex gap-2 items-center justify-center pb-6 pt-2.5 px-3 shrink-0 ${type === 'drawer' ? 'border-none shadow-none' : ''}`}>
            ...
            >
              <Share className="relative shrink-0 size-4 text-primary-foreground" />
              <p className="font-medium leading-5 relative shrink-0 text-primary-foreground text-sm capitalize">use avatar</p>
            </Button>
            <Button
              type="button"
