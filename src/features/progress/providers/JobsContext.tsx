...
const STUCK_JOB_THRESHOLD = 10 * 60 * 1000 // 10 minutes - mark as failed if stuck
...
function isJobStuck(job: Job): boolean {
  return (
    ...
    Date.now() - job.startedAt > STUCK_JOB_THRESHOLD
  )
}
...
export function JobsProvider({ children }: { children: React.ReactNode }) {
  ...
  const pollJobStatuses = useCallback(async () => {
    ...
    if (stuckJobs.length > 0) {
      ...
      setJobs((prev) =>
        prev.map((j) =>
          isJobStuck(j) ? { ...j, status: 'failed' as JobStatus, progress: 0 } : j
        )
      )
    ...
    const updates = await Promise.allSettled(
      processingJobs.map(async (job) => {
        ...
        let result: { status: JobStatus; thumbnail?: string; progress?: number }
    ...
    updates.forEach((settled) => {
      if (settled.status === "fulfilled" && settled.value) {
        const { jobId, status, thumbnail, progress } = settled.value
        ...
        if (wasProcessing && isNowReady && !notifiedJobsRef.current.has(jobId)) {
          ...
          const message =
            job.type === "likeness"
              ? "Your avatar candidates are ready! 🎉"
