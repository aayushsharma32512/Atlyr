...
import { useStudioProductOutfits, type ProductOutfitEntry } from "@/features/studio/hooks/useStudioProductOutfits"
...
export function OutfitSuggestionsView() {
  ...
  const resolvedProductId = productIdFromParams ?? selectedProductId ?? null
  ...
  const buildInspirationItem = useCallback(
    (entry: ProductOutfitEntry, index: number): InspirationItem => {
  ...
  const outfitItems = useMemo(() => {
    return (productOutfitsQuery.data ?? []).map((entry, index) => buildInspirationItem(entry, index))
  }, [buildInspirationItem, productOutfitsQuery.data])
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-start overflow-hidden px-1 pt-3">
      <div className={`flex w-full max-w-[${CARD_MAX_WIDTH}] flex-1 flex-col overflow-hidden rounded-[2rem] bg-card shadow-sm`}>
        <ScreenHeader title="More styles" onAction={handleBack} />
        <div className="flex flex-1 min-h-0 flex-col overflow-hidden">
          <div className="flex flex-1 overflow-y-auto px-2 pb-20 pt-2 scrollbar-hide">
            {!hasProductId ? (
