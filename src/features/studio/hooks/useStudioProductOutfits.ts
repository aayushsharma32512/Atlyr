...
export function useStudioProductOutfits({ productId, slot, enabled = true, limit }: UseStudioProductOutfitsArgs) {
  return useQuery({
    queryKey: studioKeys.productOutfits(productId, slot),
    enabled: Boolean(productId && slot && enabled),
    queryFn: () =>
      productId && slot
        ? studioService.getOutfitsByProduct({ productId, slot, limit })
