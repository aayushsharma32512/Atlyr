...
function AllOutfitsTable() {
  const { data: outfits, isLoading, error } = useOutfitsForEnrichment()
  ...
  // Count outfits that need enrichment
  const unenrichedCount = outfits?.filter(
    (o) => o.enrichmentStatus === "not_generated"
  ).length ?? 0
